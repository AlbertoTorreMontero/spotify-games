...
if (!REDIRECT_URI) {
    throw new Error(
        "Falta SPOTIFY_REDIRECT_URI en las variables de entorno."
    );
}
...
const GAME_TRACKS = 20;

const HIPSTER_PLAYLIST_ID =
    process.env.HIPSTER_PLAYLIST_ID;
...
async function spotifyFetch(endpoint, options = {}) {
    ...
    if (!token) {
        throw new SpotifyError(
            "No hay ninguna sesión de Spotify activa.",
            401
        );
    }
    ...
    if (!response.ok) {
        ...
        if (response.status === 403) {
            throw new SpotifyError(
                "Spotify ha denegado la petición (403). " +
                "En Development Mode solo puedes usar cuentas " +
                "añadidas en el Dashboard y leer playlists tuyas " +
                "o colaborativas.",
                403
            );
        }
...
async function getPlaylistInfo(playlistId) {
    ...
    if (total === null) {
        ...
        throw new SpotifyError(
            "Spotify solo deja leer el contenido de playlists " +
            "tuyas o colaborativas. Guarda una copia en tu cuenta " +
            "y vuelve a probar.",
            403
        );
    }
...
async function getRandomPlaylistTracks(
    playlistId,
    amount = GAME_TRACKS
) {
...
app.get("/login", (req, res) => {
    ...
    const spotifyUrl =
        `${SPOTIFY_ACCOUNTS}/authorize?${params.toString()}`;
...
app.get(
    ...
    route(async (req, res) => {
        ...
        const {
            ...
            await getRandomPlaylistTracks(
                req.params.playlistId
            );

        console.log(
            `${name}: ${tracks.length} canciones`
        );
...
function startGame(room) {
    ...
    if (
        ...
        return broadcast(
            ...
            {
                message:
                    "Hacen falta al menos dos jugadores."
            }
        );
    }
    ...
    if (notReady.length) {
        ...
        return broadcast(
            ...
            {
                message:
                    "Todos tienen que elegir una playlist antes de empezar."
            }
        );
    }
...
function leaveRoom(player) {
    ...
    if (
        ...
        broadcast(
            ...
            {
                message:
                    "No quedan jugadores suficientes. Vuelta a la sala."
            }
        );
                    ...
                    case "set_playlist": {
                        ...
                        if (
                            ...
                            return send(
                                ...
                                {
                                    message:
                                        `Esa playlist solo tiene ${tracks.length} canciones con portada. Hacen falta ${MIN_TRACKS}.`
                                }
                            );
                        }
...
server.listen(
    ...
    () => {
        ...
        console.log(
            `BeatPlay en http://127.0.0.1:${PORT}`
        );
    }
);